...
import { useUndo, type UseUndoReturn } from './use-undo';
import {
  getSessions,
  getSession,
  saveSession,
...
import { STORAGE_CONFIG, DEFAULT_TEMPLATES } from './constants';
        ...
        if (sessionToLoad && mounted) {
          try {
            ...
            if (draftJson) {
              ...
              // 检查草稿是否新于当前会话，且是同一会话
              if (draft.sessionId === sessionToLoad.id && draft.timestamp > sessionToLoad.updatedAt) {
                // 提示用户恢复草稿
  ...
  const saveNow = useCallback(async () => {
    ...
    try {
      ...
      const updatedSession: Session = {
        id: sessionId,
        name: existingSession?.name || `Session ${sessionId.slice(0, 6)}`,
