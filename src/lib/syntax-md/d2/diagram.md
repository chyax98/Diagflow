...
- 数据相关: cylinder, queue, package, stored_data, database
- 流程控制: diamond, oval, circle, hexagon
- 特殊形状: step, callout, person, cloud
- 形状语法: 节点名.shape: circle
- 1:1比例形状: circle, square（宽高始终相等）

完整样式系统：
- 填充: style.fill: 颜色/渐变（如"#f9f", "linear-gradient(#f69d3c, #3f87a6)"）
...
根级样式（整体图表）：
- 背景: style.fill: 颜色
- 边框: style.stroke: 颜色, style.stroke-width: 数字
...
- ❌ 不支持tala布局（用direction替代）
- ❌ 不支持变量系统、导入系统、泛型语法<T>
- ❌ 不支持sql_table和class等高级表格类型（渲染可能不完整）
- ⚠️ 渐变色在某些浏览器可能显示异常
...
基础形状: {
  r: rectangle {shape: rectangle}
  s: 正方形 {shape: square}
  p: 页面 {shape: page}
  pa: 平行四边形 {shape: parallelogram}
  d: 文档 {shape: document}
